...
public class TMapCRDT implements TMapInterface {

    private GenericCRDT<MapOperation> crdt;

    public TMapCRDT(boolean cache) {
    ...
    public boolean contains(String key) {
        ...
        for (MapOperation op: nonObs) {
            if (op.getKey().equals(key) && op.getType() == AddMapOperation.ADD)
                return true;
            if (op.getKey().equals(key) && op.getType() == RemMapOperation.REMOVE)
                return false;
        }
        return false;
    }
    ...
    //TODO: Remove
    public Set<MapOperation> getOps() {
        return crdt.getOps();
    }
